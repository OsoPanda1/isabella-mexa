...
async function executeProviderLocal(
  ...
  // For local providers, attempt PennyLane bridge execution
  if (implementation.startsWith("PENNYLANE")) {
    // Simulate execution result (in production: spawn Python worker)
    const isAnalytic = request.mode === "analytic";
    const numQubits = request.wires;
